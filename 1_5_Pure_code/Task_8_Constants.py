"""
Для данного задания я опишу использование констант в своем последнем проекте
дашборд с анализом данных перевозок грузов по ЖД (pandas, plotly)

Константы выделены в отдельный файл и там в класс
"""
...
class Constants:
    ...
    # Это пример присвоения константам длинных названий столбцов в pandas
    ...
    # Года на которые идет расчет - постоянно меняются у заказчика - используются везде
    YEARS = [2025,  2026, 2027, 2028, 2029, 2030, 2031, 2032, 2033, 2034, 2035]
    LITTLE_YEARS = [2025,  2026, 2027, 2028, 2029, 2030]


    # набор параметров, которые надо учитывать при пересчете - используется на каждой странцие
    INPUTS = [
    ...
    # параметры, по которым происходит группировка
